...
$cols = $con->query("SELECT COLUMN_NAME FROM information_schema.COLUMNS WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'doctors' AND COLUMN_NAME IN ('reset_token','token_expiry')");
$existing = [];
while ($c = $cols->fetch_row()) $existing[] = $c[0];
if (!in_array('reset_token',  $existing)) $con->query("ALTER TABLE `doctors` ADD COLUMN `reset_token` VARCHAR(64) NULL DEFAULT NULL");
if (!in_array('token_expiry', $existing)) $con->query("ALTER TABLE `doctors` ADD COLUMN `token_expiry` DATETIME NULL DEFAULT NULL");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['send_token'])) {
    ...
    $stmt  = $con->prepare("SELECT did FROM doctors WHERE demail = ?");
    ...
    if ($stmt->num_rows === 1) {
        ...
        $upd = $con->prepare("UPDATE doctors SET reset_token = ?, token_expiry = ? WHERE demail = ?");
        ...
        $message = "No doctor account found with that email address.";
        $msgType = 'error';
    }
    $stmt->close();
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['reset_password'])) {
    ...
    if ($newPass !== $confirm) {
        ...
        $stmt = $con->prepare("SELECT did FROM doctors WHERE reset_token = ? AND token_expiry > ?");
        ...
        if ($stmt->num_rows === 1) {
            ...
            $upd = $con->prepare("UPDATE doctors SET dpassword = ?, reset_token = NULL, token_expiry = NULL WHERE reset_token = ?");
